...
• Course registration
    ...
    async def grievance_description(
        self, update: Update, context: ContextTypes.DEFAULT_TYPE
    ):
...
Thank you for using VIT Freshman Survival Kit! 🎓
    ...
    async def handle_message(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        """Handle regular messages - IMPROVED with better NLP"""
        ...
        if intent == "greeting":
        ...
        else:  # FAQ search - NOW USES IMPROVED NLP
            # Pass NLP processor to database for better matching
            matches = self.db.search_faq(user_message, self.nlp)
            ...
            if matches:
                ...
                query_type = self.nlp.extract_question_type(user_message)

                response = (
                    f"🤔 I couldn't find a specific answer to your {query_type} question.\n\n"
                    "*Here's what you can do:*\n"
                    "• Type /faq to browse all available questions\n"
                    "• Try using different words (e.g., 'library hours' instead of 'when library open')\n"
                    "• Submit a query using /grievance if it's a specific issue\n\n"
                    "💡 *Tip:* I understand questions like:\n"
    ...
    def run(self):
        ...
        grievance_handler = ConversationHandler(
            ...
            states={
                CATEGORY: [
                    MessageHandler(
                        filters.TEXT & ~filters.COMMAND, self.grievance_category
                    )
                ],
                DESCRIPTION: [
                    MessageHandler(
                        filters.TEXT & ~filters.COMMAND, self.grievance_description
                    )
                ],
            },
